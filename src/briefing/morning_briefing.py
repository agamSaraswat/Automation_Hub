...
def _briefing_system_prompt(name: str) -> str:
    return f"""You are {name}'s personal briefing assistant.
...
"""
    ...
    prompt = f"""Create {user_name}'s morning briefing using this data:
...
"""
    ...
    try:
        client = ClaudeClient()
        briefing = client.complete(prompt, system=_briefing_system_prompt(user_name), temperature=0.6)
    except Exception as exc:
        logger.error("Claude briefing generation failed: %s", exc)
        # Fallback: plain text briefing
        briefing = (
            f"☀️ Good morning, {user_name}! — {now}\n\n"
