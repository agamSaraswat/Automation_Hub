...
def _get_master_resume_path():
    ...
    resume_rel = profile.get("master_resume_path", "master_resume/agam_master_resume.md")
    return REPO_ROOT / resume_rel

def _build_tailoring_prompt(profile: dict) -> str:
    name = profile.get("name", "the applicant")
    return f"""You are an expert ATS resume optimizer and career coach.
You help {name} tailor their resume and cover letter for specific job descriptions.

RULES:
- NEVER fabricate experience, skills, or metrics the applicant doesn't have.
...
def _load_resume() -> str:
    ...
    return _get_master_resume_path().read_text(encoding="utf-8")
...
def tailor_for_job(
    ...
    # Load active profile for contact info
    profile = _get_profile()
    p_name = profile.get("name", "Applicant")
    p_email = profile.get("email", "")
    p_phone = profile.get("phone", "")
    p_location = profile.get("location", "")
...
10. <keyword>
MISSING_SKILLS:
- <skill the applicant doesn't have but JD requires>
MATCH_ASSESSMENT: <1-2 sentence honest assessment>
"""
    system_prompt = _build_tailoring_prompt(profile)
    ...
    resume_prompt = f"""Tailor {p_name}'s resume for this specific job.
...
- Do NOT add skills or experiences the applicant doesn't have.
- Output a complete markdown resume.
"""
    tailored_resume = client.complete(resume_prompt, system=system_prompt, temperature=0.3)

    # ── Step 3: Generate cover letter ──
    cover_prompt = f"""Write a cover letter for {p_name} applying to "{title}" at "{company}".
...
- Open with why {company} specifically interests {p_name}
